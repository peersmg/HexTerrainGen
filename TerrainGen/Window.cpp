...
void Window::SetCamera(int camera)
{
  if (camera < m_cameras.size())
  {
    m_window->setView(m_cameras[camera]->GetView());
  }
  else
  {
    OutputLog::GetInstance().AddLine("Cant set camera, camera does not exist!", MessageType::ERROR);
  }
}