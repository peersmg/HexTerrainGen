...
void SceneManager::SetScene(std::string sceneId, InitialisationData data)
{
  if (SceneExists(sceneId) && GetScene(sceneId) != m_pCurScene)
  {
    if (m_pCurScene == nullptr)
    {
      ...
      m_pCurScene->LoadScene(data);
    }
    else 
    {
      OutputLog::GetInstance().AddLine("Setting new scene", MessageType::MESSAGE);
      Game::instance.GetWindow()->CleanupCameras();

      m_pCurScene->SetActive(false);
      m_pCurScene = nullptr;
      m_scenes[sceneId]->SetActive(true);
      m_pCurScene = m_scenes[sceneId];
      m_pCurScene->LoadScene(data);
    }
  }
