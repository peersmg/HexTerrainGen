...
void InputManager::SampleKeyboard()
{
  ...
  while (Game::instance.GetWindow()->GetRenderWindow()->pollEvent(currentEvent))
  {
    ...
    if (currentEvent.type == sf::Event::KeyPressed && m_keyState[currentEvent.key.code] != ButtonState::DOWN)
    {
...
bool InputManager::MouseOver(sf::FloatRect Rect, int camera)
{
  sf::Vector2i mousePos = GetMousePosition(camera);
